...
internal class RegionRepository(
    ...
    override suspend fun requestAllRegions(): List<RegionModel> =
        service.requestRegionData()

    override suspend fun requestRegionsPerCountry(countryCode: String): List<RegionModel> =
        service.requestRegionData().filter { it.countryCode == countryCode }
}